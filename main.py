...
def initalize(cols, rows, showGrid=False):
   ...
   for x in range(len(grid.getGrid())):
      for y in range(len(grid.getGrid()[x])):
         if (x == 0 or x == len(grid.getGrid())-1 or y == 0 or y == len(grid.getGrid()[x])-1):
        ...
        if pygame.mouse.get_pressed()[0]: #See if the user has clicked or dragged their mouse
            try:
                ...
                if pos[1] >= grid.height: # If the mouse is below the main drawing grid
                    if pos[0] >= tools.startx and pos[0] <= tools.startx + tools.width and pos[1] >= tools.starty and pos[1] <+ tools.starty + tools.height: #If the mouse is clicking on the tools grid
                        replace = False
                        doFill = False
                    ...
                    if replace: #If we have the replace tool selected then replace the color
                       ...
                       if clicked.color != color:
                          ...
                          #####EXP CODE#####
                           shade_obj = Shade(clicked,grid,color,clicked.color,rows,cols)
                           shade_obj.shade()
                           del shade_obj
                          ##################
                           pygame.display.update()
